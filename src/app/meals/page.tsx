...
export default function MealsPage() {
  ...
  const { meals, totals, loading, refresh, deleteMeal } =
    useDailyLog(date);
  ...
  return (
    <div className="space-y-5 px-4 py-6">
      ...
      <div className="card flex justify-between text-sm">
        <span>Total: {Math.round(safeTotals.calories)} kcal</span>
        <span>
          P{Math.round(safeTotals.protein)} C
          {Math.round(safeTotals.carbs)} F
          {Math.round(safeTotals.fat)}
        </span>
      </div>
